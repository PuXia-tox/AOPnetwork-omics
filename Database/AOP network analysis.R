AopNetworkPlot<-function(data,cmke,sample_name){
  ...
  ## ---- Filter Valid Paths ----
  qAOP_path <- sapply(cmke,function(x)x$path)
  ...
  pdf_filename <- paste0('AOP_Network_Overview_', sanitized_base_name, '.pdf')
  ...
  legend("bottomleft",
         legend = c("KE", "Neuro_AO", "MIE", "AO", "Perturbed Path"),
         col    = c("lightblue", "yellow", "green", "red", "purple"),
         pch    = 19, pt.cex = 1.5)
  
  dev.off()
  
  return(paste0('matched KEs:',length(ke.l)))
