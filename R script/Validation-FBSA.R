...
# ---- DEG Files ----
DEG_list <- list()
DEG_list[[1]] <- read_excel(paste0(deg_path,'FBSA.xlsx'),  na='NA') %>% as.matrix()
sample_names <- c('FBSA')
...
for(i in 1:nrow(data)){
  my_mat[,which(colnames(my_mat) == data[i,1])] <- log10(data[i,2]/299.12)+3
}

predictions <- predict(plsMods[[1]], my_mat)
nComps <- plsMods[[1]]$ncomp.selected
predicted_value <- 10^(predictions$y.pred[1, nComps, 1])/1000*299.12
