my_list <- c('DNBP','DIBP','DGD','DINP','DINCH','GTA')


input_names <- my_list[6]
...
MW[[5]] <- 424.666
MW[[6]] <- 	342.391
names(MW) <- c('GTA','DNBP','DIBP','DINP','DINCH','DGD')
MW <- unlist(MW[sapply(strsplit(input_names,'_'),function(x)x[1])])


# ---- Enrichment Loop ----
mmm <- 1
DEG <- DEG_list[[mmm]][, c(1, 2)]
...
for(i in 1:nrow(data)){
  my_mat[,which(colnames(my_mat) == data[i,1])] <- log10(data[i,2])
}
predictions <- predict(plsMods_neuro_tmke, my_mat)
nComps <- plsMods[[1]]$ncomp.selected
predicted_value_neuro_tmke <- 10^(predictions$y.pred[1, nComps, 1])
...
nComps <- plsMods[[1]]$ncomp.selected
predicted_value_allke <- 10^(predictions$y.pred[1, nComps, 1])



print(paste0('neuro+cmke model: ', round(predicted_value_neuro_tmke,2)))
print(paste0('all KE-based model: ', round(predicted_value_allke,2)))
