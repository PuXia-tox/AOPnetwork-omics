...
for(i in 1:nrow(data)){
  my_mat[,which(colnames(my_mat) == data[i,1])] <- log10(data[i,2]/MW)+3
}
predictions <- predict(plsMods_neuro_tmke, my_mat)
nComps <- plsMods[[1]]$ncomp.selected
predicted_value_neuro_tmke <- 10^(predictions$y.pred[1, nComps, 1])/1000*MW
...
nComps <- plsMods[[1]]$ncomp.selected
predicted_value_allke <- 10^(predictions$y.pred[1, nComps, 1])/1000*MW
